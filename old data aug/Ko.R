setwd("C:/Users/rjreyno4/Documents/DAG validation/Bone/Summer 2023 interns")

## Ko
nastring <- c("           *","epiphysis broken")  # things we want R to read as NA

# Read raw files
ko1 <- read.csv("LSDS-40_Bone_Biomechanical_LDSD-40_biomechanical_KoTRANSFORMED.csv",  header=T, stringsAsFactors=F) 
ko2 <- read.csv("LSDS-40_histomorphometry_LSDS-40_histomorphometry_KoTRANSFORMED.csv", header=T, stringsAsFactors=F,na.strings=nastring)
ko3 <- read.csv("LSDS-40_microCT_LSDS-40_microCT_KoTRANSFORMED.csv",                   header=T, stringsAsFactors=F,na.strings=nastring)
ko4 <- read.csv("LSDS-41_peripheral_quantitative_computed_tomography_pQCT_LSDS-41_pQCT_KoTRANSFORMED.csv", header=T, stringsAsFactors=F)

# Subest to needed columns/rows
ko1 <- ko1[,c(1,3:4,8:10)]
ko2 <- ko2[!(is.na(ko2$ï..Source.Name)),c(1,7:11)]
